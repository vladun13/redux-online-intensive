...
describe('auth actions:', () => {
	...
	test('authenticateAsync', () => {
		expect(authActions.authenticateAsync(__.credentials)).toEqual({
			type: 		types.AUTHENTICATE_ASYNC,
		});
	});

	test('initializeAsync', () => {
		expect(authActions.initializeAsync(__.credentials)).toEqual({
			type: 		types.INITIALIZE_ASYNC,
		});
	});

	test('logoutAsync', () => {
		expect(authActions.logoutAsync(__.credentials)).toEqual({
